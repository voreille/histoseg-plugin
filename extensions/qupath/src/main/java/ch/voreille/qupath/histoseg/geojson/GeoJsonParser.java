...
public class GeoJsonParser {
    ...
    public static List<PathObject> parseFeatureCollection(JsonObject featureCollection) {
        return featureCollectionObjectToAnnotations(featureCollection, null, null);
    }

    public static List<PathObject> parseFeatureCollection(
            JsonObject featureCollection,
            String fallbackClassName,
            String fallbackObjectName
    ) {
        return featureCollectionObjectToAnnotations(
                featureCollection,
                fallbackClassName,
                fallbackObjectName
        );
    ...
    public static List<PathObject> parseWsiResponse(JsonObject root) {
        List<PathObject> objects = new ArrayList<>();

        JsonObject tissue = getObject(root, "tissue");
        if (tissue != null) {
            objects.addAll(parseFeatureCollection(tissue, "Tissue", "Tissue"));
        }
        ...
        if (outputs != null) {
            for (Map.Entry<String, JsonElement> entry : outputs.entrySet()) {
                ...
                objects.addAll(parseFeatureCollection(
    ...
    private static PathObject createAnnotationObject(
            ROI roi,
            String className,
            String objectName
    ) {
