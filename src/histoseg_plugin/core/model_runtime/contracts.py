...
RepresentationKind = Literal["logits", "probs", "embeddings", "labels"]
...
class OutputHeadSpec(BaseModel):
    ...
    def validate_head(self) -> "OutputHeadSpec":
        if self.task_type == "semantic_segmentation" and self.semantic_source == "static":
            if self.num_classes is None:
                raise ValueError(
                    "static semantic_segmentation heads require num_classes"
                )
            if self.num_classes <= 0:
                raise ValueError("num_classes must be > 0")
            if len(self.labels) != self.num_classes:
                raise ValueError(
                    "num_classes must match number of labels for static semantic_segmentation heads"
                )
        ...
        if self.semantic_source == "context_dynamic":
            if self.labels and self.num_classes is not None and len(self.labels) != self.num_classes:
                raise ValueError(
                    "for context_dynamic outputs, if labels and num_classes are both provided, they must match"
                )
...
class ClientOutputSpec(BaseModel):
    ...
    def validate_client_output(self) -> "ClientOutputSpec":
        if self.task_type == "semantic_segmentation" and self.semantic_source == "static":
            ...
            if len(self.labels) != self.num_classes:
                raise ValueError(
                    "num_classes must match number of labels for static semantic_segmentation client outputs"
                )
...
class DerivedOutputSpec(BaseModel):
    ...
    output: ClientOutputSpec

    @model_validator(mode="after")
    def validate_derived_output(self) -> "DerivedOutputSpec":
        if not self.type:
...
class InferenceSpec(BaseModel):
    ...
    def validate_inference(self) -> "InferenceSpec":
        if self.use_amp and self.amp_dtype is None:
            raise ValueError("amp_dtype must be set when use_amp=True")
        return self
...
class ModelManifest(BaseModel):
    ...
    postprocessing: PostprocessingSpec | None = None
    ...
    def validate_manifest(self) -> "ModelManifest":
        ...
        if self.postprocessing is not None:
            for derived_name, derived_spec in self.postprocessing.derived_outputs.items():
                for input_alias, head_name in derived_spec.inputs.items():
                    if head_name not in self.output:
                        raise ValueError(
                            f"derived output '{derived_name}' references unknown raw output "
                            f"'{head_name}' via input '{input_alias}'"
                        )
