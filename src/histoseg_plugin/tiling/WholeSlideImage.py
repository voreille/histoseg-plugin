...
from histoseg_plugin.utils.file_utils import load_pkl, save_pkl
...
from histoseg_plugin.tiling.wsi_utils import (
    coord_generator,
    initialize_hdf5_bag,
    isBlackPatch,
    isWhitePatch,
...
class WholeSlideImage(object):
    ...
    def initXML(self, xml_path):
        ...
        self.contours_tumor = [
            _createContour(coord_list) for coord_list in annotations
        ]
        self.contours_tumor = sorted(self.contours_tumor,
                                     key=cv2.contourArea,
                                     reverse=True)
            ...
            for idx, annot_group in enumerate(annot):
                ...
                if annot_group['type'] == 'Polygon':
                    for idx, contour in enumerate(contour_group):
                        contour = np.array(contour).astype(np.int32).reshape(
                            -1, 1, 2)
    ...
    def segmentTissue(self,
        ...
        def _filter_contours(contours, hierarchy, filter_params):
            ...
            for hole_ids in all_holes:
                ...
                unfilered_holes = sorted(unfiltered_holes,
                                         key=cv2.contourArea,
                                         reverse=True)
        ...
        if len(keep_ids) > 0:
            ...
            contour_ids = set(np.arange(len(
                self.contours_tissue))) - set(exclude_ids)
    ...
    def visWSI(self,
        ...
        img = np.array(
            self.wsi.read_region(top_left, vis_level,
                                 region_size).convert("RGB"))

        if not view_slide_only:
            offset = tuple(-(np.array(top_left) * scale).astype(int))
            line_thickness = int(line_thickness *
                                 math.sqrt(scale[0] * scale[1]))
            if self.contours_tissue is not None and seg_display:
                if not number_contours:
                    cv2.drawContours(img,
                                     self.scaleContourDim(
                                         self.contours_tissue, scale),
                                     -1,
                                     color,
                                     line_thickness,
                                     lineType=cv2.LINE_8,
            ...
            if self.contours_tumor is not None and annot_display:
                cv2.drawContours(img,
                                 self.scaleContourDim(self.contours_tumor,
                                                      scale),
                                 -1,
                                 annot_color,
                                 line_thickness,
    ...
    def createPatches_bag_hdf5(self,
                               save_path,
                               patch_level=0,
                               patch_size=256,
                               step_size=256,
        ...
        for idx, cont in enumerate(contours):
            patch_gen = self._getPatchGenerator(cont, idx, patch_level,
            ...
            if self.hdf5_file is None:
                ...
                file_path = initialize_hdf5_bag(first_patch,
    ...
    def isInHoles(holes, pt, patch_size):
        for hole in holes:
            if cv2.pointPolygonTest(
                    hole, (pt[0] + patch_size / 2, pt[1] + patch_size / 2),
                    False) > 0:
                return 1
    ...
    def _assertLevelDownsamples(self):
        ...
        for downsample, dim in zip(self.wsi.level_downsamples,
    ...
    def process_contours(self,
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            asset_dict, attr_dict = self.process_contour(
                cont, self.holes_tissue[idx], patch_level, save_path,
    ...
    @staticmethod
    def process_coord_candidate(coord, contour_holes, ref_patch_size,
                                cont_check_fn):
        if WholeSlideImage.isInContours(cont_check_fn, coord, contour_holes,
                                        ref_patch_size):
            return coord
        else:
            return None
    ...
    def visHeatmap(self,
        ...
        scale = [1 / downsample[0],
                 1 / downsample[1]]  # Scaling from 0 to desired level
        ...
        patch_size = np.ceil(np.array(patch_size) *
                             np.array(scale)).astype(int)
        ...
        for idx in range(len(coords)):
            ...
            overlay[coord[1]:coord[1] + patch_size[1],
                    coord[0]:coord[0] + patch_size[0]] += score
            # accumulate counter
            counter[coord[1]:coord[1] + patch_size[1],
                    coord[0]:coord[0] + patch_size[0]] += 1

        if binarize:
            print('\nbinarized tiles based on cutoff of {}'.format(threshold))
            print('identified {}/{} patches as positive'.format(
                count, len(coords)))
        ...
        if binarize:
            overlay[~zero_mask] = np.around(overlay[~zero_mask] /
                                            counter[~zero_mask])
        else:
        ...
        if blur:
            overlay = cv2.GaussianBlur(
                overlay,
                tuple((patch_size * (1 - overlap)).astype(int) * 2 + 1), 0)

        if segment:
            tissue_mask = self.get_seg_mask(region_size,
                                            scale,
                                            use_holes=use_holes,
        ...
        for idx in range(len(coords)):
            ...
            if score >= threshold:
                ...
                # attention block
                raw_block = overlay[coord[1]:coord[1] + patch_size[1],
                                    coord[0]:coord[0] + patch_size[0]]

                # image block (either blank canvas or orig image)
                img_block = img[coord[1]:coord[1] + patch_size[1],
                                coord[0]:coord[0] + patch_size[0]].copy()

                # color block (cmap applied to attention block)
                color_block = (cmap(raw_block) * 255)[:, :, :3].astype(
                    np.uint8)

                if segment:
                    # tissue mask block
                    mask_block = tissue_mask[coord[1]:coord[1] + patch_size[1],
                                             coord[0]:coord[0] + patch_size[0]]
                ...
                # rewrite image block
                img[coord[1]:coord[1] + patch_size[1],
                    coord[0]:coord[0] + patch_size[0]] = img_block.copy()
        ...
        if blur:
            img = cv2.GaussianBlur(
                img, tuple((patch_size * (1 - overlap)).astype(int) * 2 + 1),
                0)
    ...
    def block_blending(self,
        ...
        for x_start in range(top_left[0], bot_right[0],
                             block_size_x * int(downsample[0])):
            for y_start in range(top_left[1], bot_right[1],
                                 block_size_y * int(downsample[1])):
                ...
                blend_block_size = (x_end_img - x_start_img,
                                    y_end_img - y_start_img)

                if not blank_canvas:
                    # 4. read actual wsi block as canvas block
                    pt = (x_start, y_start)
                    canvas = np.array(
                        self.wsi.read_region(pt, vis_level,
                                             blend_block_size).convert("RGB"))
                else:
                    # 4. OR create blank canvas block
                    canvas = np.array(
                        Image.new(size=blend_block_size,
                                  mode="RGB",
                                  color=(255, 255, 255)))

                # 5. blend color block and canvas block
                img[y_start_img:y_end_img,
                    x_start_img:x_end_img] = cv2.addWeighted(
    ...
    def get_seg_mask(self, region_size, scale, use_holes=False, offset=(0, 0)):
        ...
        print('detected {}/{} of region as tissue'.format(
