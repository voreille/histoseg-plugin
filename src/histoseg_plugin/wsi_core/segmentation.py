...
def segment_tissue(
    ...
    if seg_level < 0 or seg_level >= wsi.level_count:
        seg_level = wsi.level_count - 1
        logging.warning(
            f"seg_level {seg_level} out of range, using {seg_level} instead"
        )
    ...
    hierarchy = np.squeeze(hierarchy, axis=0)[
        :, 2:
    ]  # (N,2) -> (next, prev) unused here; we use parent in _filter
...
def _filter_contours(contours, hierarchy, fp) -> Tuple[List[Array], List[List[Array]]]:
    filtered_idxs = []
    all_holes = []
    ...
    for holes_ids in all_holes:
        ...
        unfiltered = sorted(unfiltered, key=cv2.contourArea, reverse=True)[
            : fp["max_n_holes"]
        ]
